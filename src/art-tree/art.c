 ...
 */
int art_search(art_tree *t, unsigned char *key, int key_len) {
    art_node **child;
    art_node *n = t->root;
...
static art_leaf* make_leaf(unsigned char *key, int key_len, int value) {
...
static int recursive_insert(art_node *n, art_node **ref, unsigned char *key, int key_len, int value, int depth, int *old) {
 ...
 */
int art_insert(art_tree *t, unsigned char *key, int key_len, int value) {
    int old_val = 0;
    int old = recursive_insert(t->root, &t->root, key, key_len, value, 0, &old_val);
    if (!old_val) t->size++;
    return old;
}
 ...
 * the value pointer is returned.
 */
int art_delete(art_tree *t, unsigned char *key, int key_len) {
