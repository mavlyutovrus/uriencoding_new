...
{
    ahtable_t* table = (ahtable_t*)malloc_or_die(sizeof(ahtable_t));
    table->flag = 0;
    table->c0 = table->c1 = '\0';
    ...
    table->slots = (unsigned char**)malloc_or_die(n * sizeof(slot_t));
    memset(table->slots, 0, n * sizeof(slot_t));

    table->slot_sizes = (size_t*)malloc_or_die(n * sizeof(size_t));
...
void ahtable_clear(ahtable_t* table)
{
    ...
    table->slots = (unsigned char**)realloc_or_die(table->slots, table->n * sizeof(slot_t));
    memset(table->slots, 0, table->n * sizeof(slot_t));

    table->slot_sizes = (size_t*)realloc_or_die(table->slot_sizes, table->n * sizeof(size_t));
...
static void ahtable_expand(ahtable_t* table)
{
    ...
    /* allocate slots */
    slot_t* slots = (unsigned char**)malloc_or_die(new_n * sizeof(slot_t));
    size_t j;
    for (j = 0; j < new_n; ++j) {
        if (slot_sizes[j] > 0) {
     ...
     * */
    slot_t* slots_next = (unsigned char**)malloc_or_die(new_n * sizeof(slot_t));
...
static value_t* get_key(ahtable_t* table, const char* key, size_t len, bool insert_missing)
{
    ...
    if (insert_missing) {
        ...
        table->slots[i] = (slot_t)realloc_or_die(table->slots[i], new_size);
...
static ahtable_sorted_iter_t* ahtable_sorted_iter_begin(const ahtable_t* table)
{
    ...
    i->table = table;
    i->xs = (slot_t*)malloc_or_die(table->m * sizeof(slot_t));
    i->i = 0;
