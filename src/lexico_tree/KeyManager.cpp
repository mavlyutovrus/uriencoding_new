...
Leaf::Leaf(unsigned long int _id) {
	id = _id;
//	tid = NULL;
}
...
class Node * Node::Add(char *part1, const char *part2, common_unordered_map< KEY_ID, class Node * >* inv) {
	size_t n;
	class Node *uri_id_tmp = NULL;
	unsigned short c = 0;
	class Node **nx, **newnext;

	class Leaf *l = new class Leaf(inv->size()+1); // why elements.size()+1
...
//		cerr << "uri_id_tmp == NULL: " << this->uri << "\t" << part2 << endl;
	...
	uri_id_tmp->prev = this;
	uri_id_tmp->leaf = l;
//	elements.insert(pair<KEY_ID, class Node *>(l->id, uri_id_tmp) ); // I cOMMENTED THIS OUT
	inv->insert(pair<KEY_ID, class Node *>(l->id, uri_id_tmp) ) ;
	this->next[c] = uri_id_tmp;
	return uri_id_tmp;
}

void Node::FindAdd(const char* part, bool *done, class Node ** newNode, common_unordered_map< KEY_ID, class Node * >* inv) {
	if (*done)
		return;
	class Node **nx;
	char *tmp1;
...
//			cerr << "----->2 tmp1 & tmp2: " << tmp1 << "\t" << tmp2 << endl;
			nx = next;
			while (*nx != NULL) {
				(*nx)->FindAdd(tmp2, done, newNode, inv);
				if (*done) {
					return;
				}
				nx++;
...
//		cerr << "----->1 tmp3 & tmp2: " << tmp1 << "\t" << tmp2 << endl;
		...
		*done = true;
		...
		if (leaf == NULL) {
//			cerr << "----->2 tmp1 & tmp2: " << uri  << "\t" << part << endl ;
			*newNode = Add(uri, part, inv);
			*done = true;
		} else {
			*newNode = this;
			*done = true;
...
KEY_ID KeyManager::GetId(KEY_ID seq) {
	KEY_ID tmp;
	//tmp = (KEY_ID) << (64-MASK);  MASK NOT DECLARED
	return seq | tmp;
}

//TYPE_ID Node::GetType(KEY_ID id) {
...
void Node::MakeInverseIndex(common_unordered_map< KEY_ID, class Node * >* elements) {//, multimap<Leaf*, TYPE_ID>* data) {
...
//		}
		...
		//size_t	nn = strlen(leaf->id) + 1;

			leaf->id = elements->size()+1;//elements.size()+1;
			elements->insert(pair<KEY_ID, class Node *>(leaf->id, this) );
...
Leaf* KeyManager::Add(const char* _uri) {
	...
	if (uri_id == NULL) { //first element on the list
		//uri->id index (tree)
		uri_id_tmp = new class Node();
		uri_id_tmp->uri = (char *) malloc(strlen(_uri) + 1);
		uri_id_tmp->leaf = new class Leaf(elements.size()+1);
		strcpy(uri_id_tmp->uri, _uri);
		uri_id = (class Node **) calloc(2, sizeof(class Node **));
		uri_id[0] = uri_id_tmp;
		uri_id[1] = NULL;
		newNode = uri_id_tmp;
		elements.insert(pair<KEY_ID, class Node *>(newNode->leaf->id, newNode) );
		...
		while (*uri_id_list != NULL) {
			(*uri_id_list)->FindAdd(_uri, &done, &newNode, &elements);
			uri_id_list++;
			c++;
...
void KeyManager::MakeInverseIndex() { //I MIGHT NOT NEED IT.. NOT SURE

	class Node **uri_id_list;
	if (uri_id != NULL) {
		uri_id_list = uri_id;
		while (*uri_id_list != NULL) {
			(*uri_id_list)->MakeInverseIndex(&elements);
			uri_id_list++;
		}
	}
}
...
//	class IdUri iduri(id);
//	string s;
//	KEY_ID seq = GetSeq(id);
	common_unordered_map<KEY_ID, class Node * >::iterator it;
...
// i commented this function
//map< KEY_ID, class Node * > * KeyManager::GetElementsOfType(TYPE_ID _type, vector<KEY_ID> *elems) {
